...
function Task:runAsync(delay, callback, ...)
    ...
    self.asyncTaskId = Script.SetTimerForFunction(delay, 'task_new_thread', {...})
...
function Task:runAllways(delaySec, callback, ...)
    ...
    self.asyncTaskId = Script.SetTimerForFunction((delaySec * 1000), 'task_constant_thread', self)
    return self.asyncTaskId
end

local simpleTask = setmetatable(Task, taskMeta)
